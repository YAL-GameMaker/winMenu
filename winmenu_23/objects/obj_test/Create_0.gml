...
trace(winmenu_get_max_height(m_file));

winmenu_bar_set(m_bar);

// make space for the menu bar!
call_later(3, time_source_units_frames, function() {
	// need a few-frame delay here because the window isn't visible right on Game Start
...
//var sys_first = winmenu_get_text(sys, 0, true);
//winmenu_set_text(sys, 0, true, sys_first + "!");
winmenu_add(m_sys, SysCMD.InGameMaker, "in GameMaker!");
winmenu_insert_submenu(m_sys, SysCMD.InGameMaker, false, SysCMD.CustomMenus, m_check, "Custom menus");

winmenu_bar_handler = function(_cmd) {
	trace("Menu item clicked:", _cmd, winmenu_get_text(m_bar, _cmd, false));
	switch (_cmd) {
